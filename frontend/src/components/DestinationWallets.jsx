...
function Row({ _row, setSuccessModal, setErrorModal, setError, setChangeHandle }) {
  ...
  const handleConfirmModal = async () => {
    try {
      setLoading(true);
      const response = await updateWallet({ wallet_name: walletName, wallet_id: row.id, wallet_address: walletAddress });
      setLoading(false);
  ...
  return (
    <>
      ...
      >
        <Fade in={editModal}>
          <MainCard title="Edit Destination Wallet" modal darkTitle content={false}>
            <Loader loading={loading} />
            <CardContent>
      ...
      >
        <MainCard title="Transfer Funds" modal darkTitle content={false}>
          <CardContent>
            <Typography id="modal-modal-description">Are you sure you want to {action} this wallet as destination wallet?</Typography>
          </CardContent>
          <Divider />
          <Stack direction="row" spacing={1} justifyContent="flex-end" sx={{ px: 2.5, py: 2 }}>
...
export default function DestinationWallets({ rows, setChangeHandle }) {
